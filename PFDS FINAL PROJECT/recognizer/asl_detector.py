...
class ASLDetector:
    ...
    def load_model_and_labels(self) -> None:
        ...
        try:
            ...
            logger.info("Successfully loaded ASL model from %s", self.model_path)
        except Exception as e:
            logger.error("Error loading ASL model: %s. Falling back to MOCK mode.", str(e))
            self.model_loaded = False
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Optional[str], float]:
        ...
        # Repo 2 style: we can use a 200x200 box in the center for the hand
        box_size = 200
